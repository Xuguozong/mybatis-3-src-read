 ...
 * Reflector 会缓存反射操作需要的类信息，例如：构造方法、属性名、getter/setter 方法等
...
public class Reflector {
  ...
  /**
   * 解决 getting 冲突方法。 一个属性只保留一个对应的方法
  ...
  private void addUniqueMethods(Map<String, Method> uniqueMethods, Method[] methods) {
    for (Method currentMethod : methods) {
      if (!currentMethod.isBridge()) { // 忽略 bridge 方法。参见 https://www.zhihu.com/question/54895701/answer/141623158
        // 获取方法签名
        String signature = getSignature(currentMethod);
        // check to see if the method is already known
        // if it is known, then an extended class must have
        // overridden a method
        // 当 uniqueMethods 不存在时添加
  ...
  /**
   * 返回形如 returnType#方法名:参数名1,参数名2,参数名3 的方法签名
   * @param method
   * @return
   */
